const SHELL='librariangpt-shell-v10';
const COVERS='librariangpt-covers-v1';
const ASSETS=['./','./index.html','./styles.css?v=10','./ui-v3.css?v=10','./app.js?v=10','./cover-addon.js?v=10','./ui-v3.js?v=10','./supabase-config.js','./manifest.webmanifest?v=10','./icons/icon-192.png','./icons/icon-512.png','./assets/goodreads.svg','./assets/google-books.svg','./assets/open-library.svg'];
...
self.addEventListener('fetch',event=>{if(event.request.method!=='GET')return;const url=new URL(event.request.url);
 if(isCover(url,event.request)){event.respondWith(caches.open(COVERS).then(async cache=>{const hit=await cache.match(event.request);if(hit)return hit;try{const response=await fetch(event.request);if(response.ok||response.type==='opaque')cache.put(event.request,response.clone());return response;}catch(error){return new Response('',{status:504,statusText:'Cover unavailable'});}}));return;}
