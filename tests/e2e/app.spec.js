...
test.describe('service-worker-controlled document', () => {
  ...
  test('reload keeps HTML, CSS and JavaScript on the same generation', async ({ page }) => {
    ...
    expect(await page.locator('meta[name="reading-room-generation"]').getAttribute('content')).toBe('67');
    ...
    expect(resources.every(url => new URL(url).searchParams.get('v') === '67')).toBe(true);
    expect(await page.evaluate(() => caches.keys())).toContain('reading-room-shell-v67');
  });
  ...
  test('Test Mode can request aggregate service-worker diagnostic state', async ({ page }) => {
    ...
    expect(state).toMatchObject({
      generation: '67',
      shell_cache: 'reading-room-shell-v67',
