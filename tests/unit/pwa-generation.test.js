...
test('generation 76 document, worker and built app remain coherent with no generation-75 cache reference', async () => {
  const [html, worker, built] = await Promise.all([
    readFile('index.html', 'utf8'),
    readFile('sw.js', 'utf8'),
    readFile('dist/app.js', 'utf8')
  ]);
  assert.match(html, /reading-room-generation" content="76"/);
  assert.match(html, /dist\/app\.js\?v=76/);
  assert.match(worker, /const GENERATION = '76'/);
