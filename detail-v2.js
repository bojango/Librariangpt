...
function userRatingCard(book){
  ...
  return `<button class="rating-card rating-user" type="button" data-edit-review="${book.id}"><span class="rating-logo">YOU</span><span class="rating-copy"><strong>${fmtRating(book.user_rating_5)}<small>/5</small></strong><span>Your rating · tap to edit</span></span></button>`;
}
function metadataRows(book){
  const rows=[
    ['ISBN',book.isbn13||book.isbn10],['Publisher',[book.publisher,book.imprint].filter(Boolean).join(' · ')],['Edition',[book.edition_year,book.edition_format].filter(Boolean).join(' · ')],['Edition statement',book.edition_statement],['Printing / impression',book.printing_impression],['Printer code / number line',book.number_line],['Pages',book.edition_page_count||book.total_pages],['Original publication',book.original_publication_year],['Language',book.language],['Country',book.country],['Series',book.series ? `${book.series}${book.series_order?` #${book.series_order}`:''}` : null],['Signed',book.signed===true?'Yes':book.signed===false?'No':null],['Condition',book.condition],['Dimensions',book.physical_dimensions],['Metadata source',book.edition_metadata_source]
...
function renderDetail(detail,book,ratings,rec){
  const copy=detail.querySelector('.detail-copy'); if(!copy)return;
  const progress=copy.querySelector('.progress-block'); const actions=copy.querySelector('.detail-actions');
  if(progress)progress.remove(); if(actions)actions.remove();
  ...
  copy.innerHTML=`<p class="eyebrow">${esc(book.fiction_nonfiction||'Book')}</p><h1>${esc(book.title)}</h1><div class="hero-author">${esc(book.authors||'Unknown author')}</div><div class="meta">${tags.map(t=>`<span class="badge">${esc(t)}</span>`).join('')}${rec?.match_score_10!=null?`<span class="badge accent">Predicted fit ${Number(rec.match_score_10).toFixed(1)}/10</span>`:''}</div><div class="rating-strip">${ratingHtml || '<div class="rating-empty">No public rating available yet.</div>'}</div><section class="book-synopsis"><h2>Synopsis</h2><p>${esc(synopsis)}</p></section><div class="reading-slot"></div><div class="actions-slot"></div>${hasRead?`<section class="review-panel"><div><p class="eyebrow">Your review</p><h2>${book.user_rating_5!=null?`${fmtRating(book.user_rating_5)} / 5`:'Not rated yet'}</h2>${book.review_notes?`<p>${esc(book.review_notes)}</p>`:'<p>Add notes about what worked, what did not, pacing, structure, characters, setting or anything else worth teaching the recommendation model.</p>'}</div><button class="btn" data-edit-review="${book.id}">${book.user_rating_5!=null||book.review_notes?'Edit review':'Add review'}</button></section>`:''}<div class="reading-dates"><div><small>Started</small><strong>${fmtDate(book.started_at)}</strong></div><div><small>Finished</small><strong>${fmtDate(book.completed_at)}</strong></div>${book.total_pages?`<div><small>Length</small><strong>${esc(book.total_pages)} pages</strong></div>`:''}</div>${rec?.why_recommended?`<section class="recommendation-panel"><p class="eyebrow">Librarian note</p><h2>Why it was recommended</h2><p>${esc(rec.why_recommended)}</p>${rec.outcome?`<span class="badge">Prediction: ${esc(rec.outcome)}</span>`:''}</section>`:''}<details class="metadata-accordion"><summary><span><strong>Book & edition details</strong><small>ISBN, publisher, printing, format and source data</small></span><span class="accordion-plus">+</span></summary><div class="metadata-list">${metadataRows(book)}</div></details>`;
  if(progress)copy.querySelector('.reading-slot')?.append(progress); if(actions)copy.querySelector('.actions-slot')?.append(actions);
  copy.querySelector('.reading-slot')?.classList.toggle('empty',!progress); copy.querySelector('.actions-slot')?.classList.toggle('empty',!actions);
}
async function maybeEnrich(book){
  if(book.synopsis && book.public_rating_provider)return false;
  const key=`librariangpt-content-${book.id}`; if(sessionStorage.getItem(key))return false; sessionStorage.setItem(key,'1');
  try{const {data,error}=await supabase.functions.invoke('content-enrichment',{body:{book_id:book.id}}); return !(error||data?.error);}catch{return false;}
}
async function enhanceDetail(){
  if(rendering||!app)return;
  const detail=app.querySelector('.detail-header');
  if(!detail||detail.dataset.detailV2==='loading'||detail.dataset.detailV2==='true')return;
  rendering=true; detail.dataset.detailV2='loading';
  try{
    let book=await resolveBook(detail); if(!book){delete detail.dataset.detailV2;return;}
    let extras=await loadExtras(book.id); renderDetail(detail,book,extras.ratings,extras.rec); detail.dataset.detailV2='true';
...
function openReview(book){
  ...
  modalRoot.querySelector('#precise-review-form')?.addEventListener('submit',async e=>{
    e.preventDefault(); const btn=e.currentTarget.querySelector('button[type="submit"]'); const rating=Number(e.currentTarget['precise-rating'].value); const notes=e.currentTarget['precise-notes'].value.trim()||null;
    btn.disabled=true;btn.textContent='Saving…'; const {error}=await supabase.rpc('save_book_review',{p_book_id:book.id,p_rating:rating,p_notes:notes,p_source:'frontend'});
...
async function reviewFromButton(button){const id=button.dataset.editReview;if(!id)return;const {data,error}=await supabase.from('v_library').select('*').eq('id',id).single();if(!error&&data)openReview(data);}
document.addEventListener('click',e=>{const b=e.target.closest('[data-edit-review]');if(b){e.preventDefault();reviewFromButton(b);}});
if(modalRoot){new MutationObserver(()=>{const rating=modalRoot.querySelector('#finish-form #rating');if(rating){rating.step='0.01';rating.inputMode='decimal';}const review=modalRoot.querySelector('#finish-form #review');if(review){review.placeholder='Optional review notes. These feed into future recommendations.';const label=review.closest('.field')?.querySelector('label');if(label)label.textContent='Review notes';}}).observe(modalRoot,{childList:true,subtree:true});}
