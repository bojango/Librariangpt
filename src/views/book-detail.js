...
import { cover as baseCover, esc, fmtDate, progressPct, progressText, statusPill } from '../ui/format.js';
...
export function bookDetailView(state) {
  ...
  const statuses = [...new Set([book.overall_status, book.ownership_status].filter(Boolean))];
  const tags = [book.primary_genre, book.edition_format].filter(Boolean);
