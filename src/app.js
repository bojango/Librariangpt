...
function appearanceEditorMarkup(draft, theme, section = 'preset') {
  ...
  const format = (key, value) => ['baseSize','headingAdjust','cardRadius','controlRadius','borderWidth','navHeight','navIconSize','navLabelSize'].includes(key) ? `${value}px` : ['lineHeight','letterSpacing','density'].includes(key) ? Number(value).toFixed(2) : value;
...
function openAppearanceEditor() {
  ...
  const syncAppearanceOutput = key => {
    ...
    const formatted = ['baseSize','headingAdjust','cardRadius','controlRadius','borderWidth','navHeight','navIconSize','navLabelSize'].includes(key) ? `${value}px` : ['lineHeight','letterSpacing','density'].includes(key) ? Number(value).toFixed(2) : value;
