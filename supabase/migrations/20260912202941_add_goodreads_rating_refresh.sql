create table public.rating_refresh_state (
...
create index rating_refresh_state_provider_retry_idx
...
as $$
  with candidates as (
    select distinct on (b.id)
      ...
      b.created_at
    from public.books b
    join public.library_entries le on le.book_id = b.id
    left join lateral (
...
begin
  if not exists (
    select 1 from public.library_entries le where le.book_id = p_book_id
  ) then
    return false;
  end if;
...
comment on table public.rating_refresh_state is
  'Server-maintained provider refresh attempts and retry backoff. Cached ratings remain in public_ratings.';
comment on function public.select_due_goodreads_rating_books(integer) is
  'Service-only stale-first Goodreads batch selector, capped at eight books.';
