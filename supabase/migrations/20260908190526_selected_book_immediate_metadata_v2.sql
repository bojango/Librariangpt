...
declare
  ...
  v_subtitle text := nullif(trim(p_result->>'subtitle'),'');
  v_synopsis text := nullif(trim(p_result->>'synopsis'),'');
  ...
  v_genre text := nullif(trim(p_result->>'primary_genre'),'');
  v_language text := nullif(trim(p_result->>'language'),'');
...
begin
  ...
  if v_book is not null then
    ...
    if v_book is null then
      insert into public.books(title,subtitle,original_publication_year,primary_genre,language,synopsis,cover_url_preferred,cover_source,cover_verified,metadata_source,metadata_last_updated,metadata_status,metadata_confidence,notes)
      values(
        v_title,v_subtitle,
        case when (p_result->>'publication_year') ~ '^\d{4}$' then (p_result->>'publication_year')::int else null end,
        v_genre,v_language,v_synopsis,
        v_cover,v_provider,(v_cover is not null),v_provider,current_date,
        case when v_synopsis is not null and v_cover is not null then 'partial' else 'unresolved' end,
        0.95,'Created from an explicitly selected Reading Room search result; background enrichment pending.'
      ) returning id into v_book;

      for v_name in select jsonb_array_elements_text(coalesce(p_result->'authors','[]'::jsonb)) loop
        v_name := nullif(trim(v_name),''); if v_name is null then continue; end if;
        v_idx := v_idx + 1;
      ...
      update public.books set
        subtitle=coalesce(subtitle,v_subtitle), synopsis=coalesce(synopsis,v_synopsis),
        primary_genre=coalesce(primary_genre,v_genre), language=coalesce(language,v_language),
        cover_url_preferred=coalesce(cover_url_preferred,v_cover),
        cover_source=case when cover_url_preferred is null and v_cover is not null then v_provider else cover_source end,
        cover_verified=case when cover_url_preferred is null and v_cover is not null then true else cover_verified end,
        updated_at=now()
      where id=v_book;
    end if;
    ...
    insert into public.editions(
      book_id,owned,preferred_copy,isbn10,isbn13,publisher,publication_year,language,format,page_count,
      cover_url,cover_source,cover_verified,edition_match_confidence,open_library_edition_id,open_library_work_id,google_books_volume_id,
      metadata_source,metadata_last_fetched_at,metadata_match_confidence,metadata_payload,is_reference
    ) values(
      v_book,v_owned,v_owned,v_isbn10,v_isbn13,nullif(p_result->>'publisher',''),v_year,v_language,nullif(p_result->>'format',''),v_page_count,
      ...
      nullif(p_result->>'open_library_work_id',''),
    ...
    update public.books set
      ...
      cover_source=case when cover_url_preferred is null and v_cover is not null then v_provider else cover_source end,
      cover_verified=case when cover_url_preferred is null and v_cover is not null then true else cover_verified end,
      metadata_status='partial',metadata_confidence=greatest(coalesce(metadata_confidence,0),0.95),updated_at=now()
    where id=v_book;
  end if;
  ...
  values(v_uid,v_book,'library_book_added','selected-search-result',jsonb_build_object('status',p_status,'ownership',p_ownership,'edition_id',v_edition,'provider',v_provider,'background_enrichment',true));

  return jsonb_build_object('ok',true,'already_in_library',false,'book_id',v_book,'edition_id',v_edition,'library_entry_id',v_library,'background_enrichment',true);
end;
$function$;
