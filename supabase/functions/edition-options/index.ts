...
async function fetchJson(url: string, timeoutMs = 9000) {
  ...
  try {
    const response = await fetch(url, { signal: controller.signal, headers: { 'User-Agent': 'ReadingRoom/7.1 edition browser' } });
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    const [bookResult, stateResult, editionsResult] = await Promise.all([
      ...
      admin.from('books').select('editions_status,editions_last_refreshed_at,editions_error,reference_edition_id,cover_locked,cover_url_preferred').eq('id', bookId).single(),
    ...
    if (!workId && selectedIsbn && isValidIsbn(selectedIsbn)) {
      const selectedOpenLibraryEdition = await fetchJson(`https://openlibrary.org/isbn/${encodeURIComponent(selectedIsbn)}.json`, 7000);
    ...
    if (!workId) {
      ...
      const search = await fetchJson(searchUrl.toString(), 8000);
      let best: any = null;
      let bestScore = 0;
      for (const result of Array.isArray(search?.docs) ? search.docs : []) {
        const score = similarity(title, result.title) * .76 + authorSimilarity(author, result.author_name || []) * .24;
        if (score > bestScore) {
          best = result;
          bestScore = score;
        }
    ...
    if (workId) {
      for (const offset of [0, 50]) {
        const response = await fetchJson(`https://openlibrary.org/works/${workId}/editions.json?limit=50&offset=${offset}`, 11000);
        ...
        for (const edition of rows) {
          ...
          candidates.push({
            ...
            edition_statement: clean(edition?.edition_name) || null, page_count: Number(edition?.number_of_pages || 0) || null,
    ...
    const googleQueries = [`intitle:"${title}"${author ? ` inauthor:"${author}"` : ''}`, `"${title}"${author ? ` "${author}"` : ''}`];
    let googleCount = 0;
    ...
    for (const query of googleQueries) {
      ...
      const response = await fetchJson(googleUrl.toString(), 9000);
      ...
      for (const item of Array.isArray(response?.items) ? response.items : []) {
        ...
        const titleScore = similarity(title, volume.title);
    ...
    return json({
      ...
      source_counts: { open_library: openLibraryCount, google_books: googleCount, candidates: merged.length },
