...
function modal(html,extraClass='',backdropClass=''){modalRoot.innerHTML=`<div class="modal-backdrop ${backdropClass}"><div class="modal ${extraClass}">${html}</div></div>`;modalRoot.querySelectorAll('[data-upnext-close]').forEach(b=>b.addEventListener('click',()=>modalRoot.innerHTML=''));modalRoot.querySelector('.modal-backdrop')?.addEventListener('click',e=>{if(e.target.classList.contains('modal-backdrop'))modalRoot.innerHTML='';});}
...
function openDetails(item){
 ...
 modal(`<div class="upnext-detail-shell"><button class="upnext-detail-close" type="button" data-upnext-close aria-label="Close">×</button><div class="upnext-detail-head">${coverMarkup(item,'upnext-detail-cover')}<div class="upnext-detail-meta"><p class="upnext-source ${item.source==='Manual'?'manual':'ai'}">${esc(sourceLabel(item))}</p><h2>${esc(item.title)}</h2><p class="upnext-detail-author">${esc(item.authors||'Unknown author')}</p>${item.ai_score!=null?`<p class="upnext-detail-score">${Number(item.ai_score).toFixed(1)}/10 · ${esc(item.confidence||'')} confidence</p>`:''}</div></div><div class="upnext-detail-reason"><h3>Why it’s up next</h3><p>${esc(fullReason)}</p></div><div class="upnext-detail-actions"><button class="btn btn-primary" id="upnext-read-now" type="button">Read now</button><button class="btn" id="upnext-open-book" type="button">Open book</button></div></div>`,'upnext-detail-modal','upnext-detail-backdrop');
