...
function toast(message,error=false){if(!toastNode)return;toastNode.textContent=message;toastNode.className=`toast show${error?' error':''}`;clearTimeout(toastNode._quotes40);toastNode._quotes40=setTimeout(()=>toastNode.className='toast',3400);}
...
function bindSaveForm(book,captureMethod,quoteId=null){
 modalRoot.querySelector('#quote-save-form-v40')?.addEventListener('submit',async e=>{
   ...
   try{
     ...
     closeModal();toast(quoteId?'Quote updated.':'Quote saved.');await refreshSection(book.id);
...
function confirmDelete(book,quote){
 ...
 modalRoot.querySelector('[data-quote-close]')?.addEventListener('click',closeModal);modalRoot.querySelector('#quote-delete-confirm-v40')?.addEventListener('click',async e=>{e.currentTarget.disabled=true;const {error}=await supabase.from('book_quotes').delete().eq('id',quote.id);if(error){toast(error.message||'Could not delete quote',true);e.currentTarget.disabled=false;return;}closeModal();toast('Quote deleted.');await refreshSection(book.id);});
}

function quoteCard(q){const loc=[pageLabel(q),q.chapter].filter(Boolean).join(' · ');return `<article class="saved-quote-v40" data-quote-id="${q.id}"><blockquote>${esc(q.quote_text)}</blockquote>${loc?`<p class="saved-quote-location-v40">${esc(loc)}</p>`:''}${q.note?`<p class="saved-quote-note-v40">${esc(q.note)}</p>`:''}<div class="saved-quote-actions-v40"><button type="button" data-quote-edit="${q.id}">Edit</button><button type="button" data-quote-delete="${q.id}">Delete</button></div></article>`;}
function renderSection(section,book,quotes){
 const count=quotes.length;section.innerHTML=`<div class="book-quotes-head-v40"><div><h2>Quotes & passages${count?` <span>${count}</span>`:''}</h2></div><div class="book-quotes-actions-v40"><button class="quote-scan-btn-v40" type="button" data-quote-scan>Scan page</button><button class="quote-manual-btn-v40" type="button" data-quote-manual>+ Add</button></div></div>${count?`<div class="saved-quotes-list-v40">${quotes.map(quoteCard).join('')}</div>`:`<p class="book-quotes-empty-v40">No saved passages yet.</p>`}`;
 section.querySelector('[data-quote-scan]')?.addEventListener('click',()=>scanPage(book));section.querySelector('[data-quote-manual]')?.addEventListener('click',()=>openManual(book));
 section.querySelectorAll('[data-quote-edit]').forEach(btn=>btn.addEventListener('click',()=>{const q=quotes.find(x=>x.id===btn.dataset.quoteEdit);if(q)openManual(book,q);}));section.querySelectorAll('[data-quote-delete]').forEach(btn=>btn.addEventListener('click',()=>{const q=quotes.find(x=>x.id===btn.dataset.quoteDelete);if(q)confirmDelete(book,q);}));
}

async function loadBookAndQuotes(bookId){const [bookRes,quoteRes]=await Promise.all([supabase.from('v_library').select('*').eq('id',bookId).single(),supabase.from('book_quotes').select('id,book_id,edition_id,session_id,page_start,page_end,chapter,quote_text,note,capture_method,created_at,updated_at').eq('book_id',bookId).order('page_start',{ascending:true,nullsFirst:false}).order('created_at',{ascending:true})]);if(bookRes.error)throw bookRes.error;if(quoteRes.error)throw quoteRes.error;return{book:bookRes.data,quotes:quoteRes.data||[]};}
async function refreshSection(bookId){const section=app?.querySelector(`.book-quotes-v40[data-book-id="${bookId}"]`);if(!section)return;try{const {book,quotes}=await loadBookAndQuotes(bookId);if(section.isConnected)renderSection(section,book,quotes);}catch(err){console.info('[Reading Room] quotes refresh unavailable',err?.message||err);}}

async function enhance(){
 if(!app)return;const detail=app.querySelector('.detail-header[data-library-detail="ready"][data-book-id]');if(!detail)return;const copy=detail.querySelector('.detail-copy-v4');const synopsis=copy?.querySelector('.book-synopsis');if(!copy||!synopsis)return;
 if(copy.querySelector('.book-quotes-v40'))return;
 const section=document.createElement('section');section.className='book-quotes-v40';section.dataset.bookId=detail.dataset.bookId;section.innerHTML='<div class="book-quotes-skeleton-v40">Quotes & passages</div>';synopsis.insertAdjacentElement('afterend',section);
 try{const {book,quotes}=await loadBookAndQuotes(detail.dataset.bookId);if(section.isConnected)renderSection(section,book,quotes);}catch(err){if(section.isConnected)section.innerHTML='<p class="book-quotes-empty-v40">Quotes could not be loaded.</p>';console.info('[Reading Room] quotes unavailable',err?.message||err);}
}
function schedule(){if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;enhance();});}
if(app)new MutationObserver(schedule).observe(app,{childList:true,subtree:true,attributes:true,attributeFilter:['data-library-detail','data-book-id']});
window.addEventListener('load',()=>setTimeout(schedule,650));window.addEventListener('library-data-updated',()=>setTimeout(schedule,120));document.addEventListener('visibilitychange',()=>{if(!document.hidden)setTimeout(schedule,180);});
