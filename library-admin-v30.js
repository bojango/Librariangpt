...
function toast(message,error=false){if(!toastNode)return;toastNode.textContent=message;toastNode.className=`toast show${error?' error':''}`;clearTimeout(toastNode._admin30);toastNode._admin30=setTimeout(()=>toastNode.className='toast',3600);}
...
function openAddBook(){
 addBusy=false;
 showAddModal(`<div class="add30-head"><h2>Add book</h2><p>Search first, then choose the exact result you want. Nothing is added until you tap a result.</p></div>
 <form id="add30-search-form" class="form-stack">
  ...
  <div data-add30-fields="title">
   <div class="field"><label for="add30-title">Book title</label><input class="input" id="add30-title" autocomplete="off" placeholder="e.g. Whiteout" required></div>
   <div class="field"><label for="add30-author">Author <span class="field-optional">optional</span></label><input class="input" id="add30-author" autocomplete="off" placeholder="e.g. Duncan Kyle"></div>
  </div>
...
function renderResults(ctx){
 ...
 ${results.length?`<div class="add30-results">${results.map((r,i)=>`<button type="button" class="add30-result ${r.already_in_library?'is-existing':''}" data-add30-result="${i}" ${r.already_in_library?'disabled':''}>
   <div class="add30-cover">${r.cover_url?`<img src="${esc(r.cover_url)}" alt="" loading="lazy" onerror="this.remove()">`:'<span>No cover</span>'}</div>
   <div class="add30-result-copy"><div class="add30-result-title">${esc(r.title)}</div><div class="add30-result-author">${esc((r.authors||[]).join(', ')||'Unknown author')}</div><div class="add30-result-meta">${esc(resultMeta(r))}</div><div class="add30-result-source">${r.already_in_library?'Already in your library':esc(r.provider||'Catalogue result')}</div></div>
  </button>`).join('')}</div>`:'<div class="add30-empty">Try a more specific title, add the author, or search by ISBN.</div>'}
...
function injectDelete(){
 ...
 const form=modal.querySelector('#book-admin-form'); const actions=form?.querySelector('.book-admin-actions'); const bookId=currentBookId(); if(!form||!actions||!bookId)return;
 ...
 const confirm=section.querySelector('.delete-confirm-v30');
 ...
 section.querySelector('[data-delete-confirm-v30]')?.addEventListener('click',async e=>{
  const btn=e.currentTarget;btn.disabled=true;btn.textContent='Deleting…';
  try{const {error}=await supabase.rpc('delete_library_book',{p_book_id:bookId});if(error)throw error;await clearCaches();closeModal();toast(`${title} deleted.`);setTimeout(()=>location.reload(),180);}catch(err){toast(err.message||'Could not delete book',true);btn.disabled=false;btn.textContent='Delete permanently';}
 });
}
